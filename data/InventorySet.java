...
final class InventorySet implements Inventory {
  ...
  Record checkOut(Video video) {
	...
	if(!_data.containsKey(video)||oldrecord.numOut==oldrecord.numOwned) {
    	throw new IllegalArgumentException();
    }

    _data.put(video,new RecordObj(video, oldrecord.numOwned,oldrecord.numOut+1 , oldrecord.numRentals+1));
    return oldrecord;
  ...
  Record checkIn(Video video) {
	...
	RecordObj oldrecord=((RecordObj)_data.get(video));
	if(video==null) throw new IllegalArgumentException();
	if(!_data.containsKey(video)||oldrecord.numOut< 1) {
		throw new IllegalArgumentException();
	}
	_data.put(video, new RecordObj(video,oldrecord.numOwned,oldrecord.numOut-1,oldrecord.numRentals));
	return oldrecord;
  ...
  Map clear() {
    // TODO  
	  HashMap<Video,Record>  olddata = new HashMap<Video,Record> (_data);
	  _data.clear();
      return olddata;
  }
